...
public class GymLose implements CommandExecutor{
			...
			if(context.hasAny("player")){
				...
				for(Player pl:Sponge.getServer().getOnlinePlayers()){
					if(pl.getName().equalsIgnoreCase(name)){
						GymManager.getInstance().addLoss(pl,gym);
						pl.sendMessage(Text.of(TextColors.RED,TextStyles.BOLD,"You Lost At ",p.getName(),"'s Gym!"));
						p.sendMessage(Text.of(TextColors.RED,TextStyles.BOLD,"Added A Loss For ",pl.getName()));
