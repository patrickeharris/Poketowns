...
public class RightClick {
	@Listener
	public void onRightClick(InteractBlockEvent.Secondary.MainHand e){
		if(e.getCause().root() instanceof Player){
			...
			if(Inventories.getInstance().hasInventory(p)){
				...
				if(poke=="charmander"){
					Abilities.getInstance().secondaryCharmander(p);
				}
				if(poke=="squirtle"){
					Abilities.getInstance().secondarySquirtle(p);
				}
				if(poke=="bulbasaur"){
					Abilities.getInstance().secondaryBulbasaur(p);
				}
				e.setCancelled(true);
			}
